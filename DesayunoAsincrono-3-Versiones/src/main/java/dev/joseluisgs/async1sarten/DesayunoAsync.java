package dev.joseluisgs.async1sarten;
...
public class DesayunoAsync {
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        var huevosBeiconTask = freirHuevosBeiconAsync(2, 3);
        var tostadaTask = prepararTostadaConMantequillaYMeremeladaAsync(2);

        // Esperar a que todas las tareas se completen
        CompletableFuture<Void> todasLasTareas = CompletableFuture.allOf(tostadaTask, huevosBeiconTask);

        // Manejar las tareas completadas
        todasLasTareas.thenRun(() -> {
            try {
                if (huevosBeiconTask.isDone()) {
                    System.out.println("Los huevos y el beicon están listos");
                }
    ...
    private static CompletableFuture<Void> freirHuevosBeiconAsync(int cantidad, int lonchas) {
        return CompletableFuture.supplyAsync(() -> {
